...
from neurons_engine import neurons_get_online_list
...
def create_f0f7_frame(data):
    ...
    frame.append(NEURONS_PROTOCOL_END)
    return frame
    ...
    def _frame_parse(self, data):
        ...
        for i in range(len(data)):
            if data[i] == NEURONS_PROTOCOL_HEAD:
                ...
                self._check_sum = 0
                for j in range(self._data_in_index - 1):
                ...
                if self._check_sum == self._frame_buf[self._data_in_index - 1]:
                    if self._frame_process:
                        #print("fff", self._frame_buf[:self._data_in_index])
                        self._frame_process(self._frame_buf[:self._data_in_index])
...
class neurons_protocol():
    def __init__(self):
        self._frame_c = _F0F7_frame()
        self._frame_c._register_frame_process(self.parse_frame)

    def parse_frame(self, frame, channel_info = 0x01):
        response_distributor(frame, channel_info, 0x00)


#####################################################
def __neurons_engine_send_frame(frame, copo = -1):
    uart_B.write(bytes(create_f0f7_frame(frame)))
...
import mbot2
while 1:
    print("test function run", neurons_get_online_list())
    mbot2.forward(20, 1)
