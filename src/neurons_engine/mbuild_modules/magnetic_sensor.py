...
def is_active(index = 1):
    ...
    value = neurons_async_read("m_magneto_dependent_sensor", "get_value", (), index)
    if value != None:
        return bool(value[0])
    else:
        return False
...
def get_count(index = 1):
    ...
    value = neurons_async_read("m_magneto_dependent_sensor", "get_count", (), index)
    if value != None:
        return value[0]
    else:
        return 0
...
def reset_count(index = 1):
    neurons_request("m_magneto_dependent_sensor", "reset_count", (), index)
    neurons_blocking_read("m_magneto_dependent_sensor", "get_count", (), index)
